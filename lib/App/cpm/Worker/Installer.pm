...
sub _fetch_git {
    ...
    $self->run_command($ctx, [ 'git', 'clone', @depth, $uri, $dir ]);
    ...
    my $guard = pushd $dir;
    if ($ref) {
        if (!$self->run_command($ctx, [ 'git', 'checkout', $ref ])) {
...
sub _configure {
    ...
    $self->run_timeout($ctx, $cmd, $self->{configure_timeout});
}
...
sub _build {
    ...
    $self->run_timeout($ctx, $cmd, $self->{build_timeout});
}
...
sub _test {
    ...
    $self->run_timeout($ctx, $cmd, $self->{test_timeout});
}
...
sub install {
    ...
    if ($static_builder) {
        ...
        $self->_retry($ctx, sub { $self->_build($ctx, [ $self->{perl}, "./Build" ], $meta)  })
        && ($self->{notest} || $self->_retry($ctx, sub { $self->_test($ctx, [ $self->{perl}, "./Build", "test" ], $meta) }))
        && $self->_retry($ctx, sub { $self->_install($ctx, [ $self->{perl}, "./Build", "install" ], $meta)  })
        && $installed++;
    } else {
        $self->_retry($ctx, sub { $self->_build($ctx, [ $self->{make} ], $meta)  })
        && ($self->{notest} || $self->_retry($ctx, sub { $self->_test($ctx, [ $self->{make}, "test" ], $meta) }))
        && $self->_retry($ctx, sub { $self->_install($ctx, [ $self->{make}, "install" ], $meta) })
        && $installed++;
    }
...
sub run_timeout {
    my ($self, $ctx, $cmd, $timeout) = @_;

    my $str = ref $cmd eq 'CODE' ? '' : ref $cmd eq 'ARRAY' ? "@$cmd" : $cmd;
    $ctx->log("Executing $str") if $str;
    ...
    my $runner = Command::Runner->new(
        ...
        redirect => 1,
        timeout => $timeout,
        stdout => sub { $ctx->log(@_) },
    );
    my $res = $runner->run;
    if ($res->{timeout}) {
        $ctx->log("Timed out (> ${timeout}s).");
        return;
    }
